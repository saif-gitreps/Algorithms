// 45/50 cases passed.
// update: 49/ 50 cases passed.
int kev(int n, int diff, long long sum,vector<int> a, vector<vector<int>> &dp){
	if(n < 0){
		return INT_MAX;
	}
	if(n == 0){
	...
	pick = kev(n-1, diff + a[n], sum, a, dp);
	not_pick = kev(n-1, diff, sum ,a ,dp);
